...
#if defined ( XCP_TRANSPORT_LAYER_TYPE_FLEXRAY )
  #include "frxcp_cfg.h"
  #include "frxcp_par.h"
#endif
...
/* if values kXcpMaxXXX_AG are not exsiting, set them to the kXcpMaxXXX value */
#if ! defined ( kXcpMaxCTO_AG )
  #define kXcpMaxCTO_AG                   kXcpMaxCTO
#endif
#if ! defined ( kXcpMaxDTO_AG )
  #define kXcpMaxDTO_AG                   kXcpMaxDTO
#endif
#if ! defined ( kXcpAG )
  #define kXcpAG                          1
#endif
...
#define CRM_SET_DAQ_LIST_MODE_LEN                       1
...
/****************************************************************************/
    ...
    #define EV_BYTE(x)            (xcp.Ev.EightByteField.byte_##x)
  #endif

  #define CRO_BYTE(x)             (pCmd->EightByteField.byte_##x)

  #define CRO_WORD(x)             (pCmd->w[x])
  #define CRO_DWORD(x)            (pCmd->dw[x])

  #define CRM_BYTE(x)             (xcp.Crm.EightByteField.byte_##x)
...
#if defined ( C_COMP_COSMIC_MCS12X_MSCAN12 )
/* If the Star12x with Comsic Compiler is used memory access by application has to be enabled.
   Also the paged memory model has to be enabled.
   Both options have to be enabled in order to allow XCP to access the whole memory. */
  #if  defined ( XCP_ENABLE_MEM_ACCESS_BY_APPL )
  #else
    #define XCP_ENABLE_MEM_ACCESS_BY_APPL
  #endif
  #if  defined ( XCP_MEMORY_MODEL_PAGED )
  #else
...
*/
    ...
    #if defined ( XCP_ALLIGN_WORD )
      #error "Write DAQ multiple not supported for XCP_ALLIGN_WORD!"
    #endif
  #endif
...
typedef struct tEightByteFieldTag
{
  vbittype          byte_0  : 8;
  vbittype          byte_1  : 8;
  vbittype          byte_2  : 8;
...
typedef union {
  ...
  vuint8            b[kXcpMaxDTO/kXcpAG];
} tXcpDtoByteField;

typedef struct {
  tXcpDtoByteField  XcpDtoByteField;
  vuint8            l;
} tXcpDto;
...
/****************************************************************************/
  ...
  #if ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD11 )
  typedef vuint8 tXcpChecksumSumType;    /* Data type (width) of the actually checksum. */
  #endif
  #if ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD12 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD22 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC16 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC16CCITT )
  typedef vuint16 tXcpChecksumSumType;   /* Data type (width) of the actually checksum. */
  #endif
  #if ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD14 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD24 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_ADD44 ) || \
      ( kXcpChecksumMethod == XCP_CHECKSUM_TYPE_CRC32 ) 
  typedef vuint32 tXcpChecksumSumType;   /* Data type (width) of the actually checksum. */
  #endif
#endif
...
V_MEMROM0 extern MEMORY_ROM  vuint8 kXcpMainVersion;
V_MEMROM0 extern MEMORY_ROM  vuint8 kXcpSubVersion;
V_MEMROM0 extern MEMORY_ROM  vuint8 kXcpReleaseVersion;
...
extern void XcpCommand( MEMORY_ROM vuint32* pCommand );
...
#if defined ( XCP_TRANSPORT_LAYER_TYPE_LIN )
  #define ApplXcpSend(len, msg)  XcpLinSend(len, msg)
  #if defined ( ApplXcpSendFlush )
  #else
    #define ApplXcpSendFlush()   XcpLinSendFlush()
  #endif
  #define ApplXcpInit()          XcpLinInit()
  #define ApplXcpBackground()    XcpLinBackground()
#endif

#if defined ( XCP_TRANSPORT_LAYER_TYPE_CAN_ASR )
  #define ApplXcpSend(len, msg)  CanXcp_Transmit(len, msg)
  #if defined ( ApplXcpSendFlush )
  #else
    #define ApplXcpSendFlush()
  #endif
  #define ApplXcpInit()          /* CanXcp_Init(&CanXcp_Config) done by CCL or System Init */
  #define ApplXcpBackground()    CanXcp_MainFunction()
#endif

#if defined ( XCP_TRANSPORT_LAYER_TYPE_FLEXRAY_ASR )
  #define ApplXcpInit()          FrXcp_Init()
  #define ApplXcpTLService(pCmd) FrXcp_TLService(pCmd)
  #define ApplXcpBackground()
  #define ApplXcpSend(len, msg)  (void)FrXcp_Send(len, msg)
  #if defined ( ApplXcpSendFlush )
  #else
    #define ApplXcpSendFlush()   FrXcp_SendFlush( XCP_FLUSH_CTO )
  #endif
#endif

#if defined ( XCP_TRANSPORT_LAYER_TYPE_TCPIP_ASR )
  #define ApplXcpInit()                 TcpIpXcp_Init()        
  #define ApplXcpBackground()
  #define ApplXcpSend(len, msg)         TcpIpXcp_Send(len, msg)
  #if defined ( ApplXcpSendFlush )
  #else
    #define ApplXcpSendFlush()   
  #endif
...
#else
extern void ApplXcpSend( vuint8 len, MEMORY_ROM BYTEPTR msg );
#endif
...
extern void XcpSendEvent( vuint8 evc, MEMORY_ROM BYTEPTR c, vuint8 len);
#endif
...
extern void XcpPutchar( MEMORY_ROM vuint8 c );
    #if defined ( XCP_ENABLE_SERV_TEXT_PRINTF )
extern void XcpPrint( MEMORY_ROM vuint8 *str );
    #endif
  #endif
#endif
...
#else
extern void XcpMemCpy( DAQBYTEPTR dest, MEMORY_ROM DAQBYTEPTR src, vuint8 n );
#endif
...
/* Can be redefined to meet DMA requirements */
  ...
  #else
extern void XcpSendDto( MEMORY_ROM tXcpDto *dto );
  #endif
#endif
...
/* Used only, if write protection of memory areas is required */
  ...
  #else
extern vuint8 ApplXcpCheckWriteAccess( MEMORY_ROM MTABYTEPTR address, vuint8 size );
  #endif
#endif
...
/* Used only, if read protection of memory areas is required */
  ...
  #else
extern vuint8 ApplXcpCheckReadAccess( MEMORY_ROM MTABYTEPTR address, vuint8 size );
  #endif
#endif

/* DAQ resume */
#if defined ( XCP_ENABLE_DAQ_RESUME )
extern void ApplXcpDaqResumeStore( MEMORY_ROM tXcpDaq * daq );
extern void ApplXcpDaqResumeClear(void);
extern vuint8 ApplXcpCalResumeStore(void);
extern vuint8 ApplXcpDaqResume( tXcpDaq * daq );
#endif 
...
extern vuint8 ApplXcpDisableNormalOperation( MTABYTEPTR a, vuint16 size );
extern vuint8 ApplXcpStartBootLoader( void );
...
extern vuint8 ApplXcpFlashClear( MTABYTEPTR address, vuint32 size );
/* Returns:
   XCP_CMD_OK    
   XCP_CMD_PENDING - call XcpSendCrm when done
   XCP_CMD_ERROR 
...
extern vuint8 ApplXcpFlashProgram( MEMORY_ROM BYTEPTR data, MTABYTEPTR address, vuint8 size );
/* Returns:
...
extern vuint8 ApplXcpGetSeed( MEMORY_ROM vuint8 resource, vuint8 *seed );
extern vuint8 ApplXcpUnlock( MEMORY_ROM vuint8 *key, MEMORY_ROM vuint8 length );

#endif

/* User defined service of GET_ID to transfer MAP-file names (ESCAN00008482) */
#if defined ( XCP_ENABLE_VECTOR_MAPNAMES )
extern vuint32 ApplXcpGetIdData( MTABYTEPTR *pData);
#endif
...
*/
extern vuint8 ApplXcpCheckWriteEEPROM( MTABYTEPTR addr, vuint8 size, MEMORY_ROM BYTEPTR data );
#endif
...
/* Return values for ApplXcpCheckReadEEPROM:
 ...
 XCP_CMD_DENIED  - This is not EEPROM
*/
extern vuint8 ApplXcpCheckReadEEPROM( MTABYTEPTR addr, vuint8 size, BYTEPTR data );
#endif
...
*/
extern vuint8 ApplXcpTLService( MEMORY_ROM BYTEPTR pCmd );
#endif
...
/* Return values for ApplXcpOpenCmdIf:
 ...
 XCP_CMD_UNKNOWN      - not implemented optional command
*/
extern vuint8 ApplXcpOpenCmdIf( MEMORY_ROM BYTEPTR pCmd, BYTEPTR pRes, BYTEPTR pLength );
#endif
